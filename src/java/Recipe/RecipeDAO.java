...
public class RecipeDAO {
    ...
    public static ArrayList<RecipeDTO> searchRecipes(String keyword, String searchBy) {
        ...
        try {
            ...
            if (cn != null) {
                String sql = "SELECT r.[title],r.[description],[prep_time],[cook_time],\n"
                        + "[servings],[create_at],[update_at],[cuisine_id],[category_id],[user_id],[level_id]\n";
                if (searchBy.equalsIgnoreCase("byTitle")) {
                    sql += "FROM [dbo].[Recipe] AS r\n"
                            + "WHERE title LIKE ?";
                }
                if (searchBy.equalsIgnoreCase("byCuisine")) {
                ...
                if (rs != null) {
                    while (rs.next()) {
                        ...
                        RecipeDTO recipe = new RecipeDTO(title, description, prep_time,
